        ...
        <div class="col-12 col-md-4">
          ...
          >
            <template v-if="model.items.length > 0">
              <template v-for="(item, index) in model.items" :key="item.cardId">
                <div class="row">
                  <div class="col-12">
                    <div class="ellipsis-2-lines">{{item.sideA}}</div>
                    <div class="ellipsis-2-lines">{{item.sideB}}</div>
                  </div>
...
const model = reactive<{ items: Card[] }>({ items: [] });
...
function openItem(item: Card | null = null): void {
  router.push({
    name: 'card',
    params: {
      deckId: item ? item.deckId : state.parentId,
      cardId: item ? item.cardId : '',
    },
  });
}

function confirmDeleteItem(item: Card, index: number) {
  quasar.dialog({
    title: i18n.t('dictionary.delete'),
    message: i18n.t('dialogs.deleteCard'),
    cancel: true,
    persistent: true,
  }).onOk(() => {
    state.wait = true;

    db.deleteCard(item.cardId, item.deckId).then(() => {
      refreshItems();
    }).finally(() => {
      state.wait = false;
    });
